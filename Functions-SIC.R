...
SIC <- 
  ...
  while (difference>eps & k<emmax){
    ...
    iteration <- m #c(m,gamma.hat,beta.hat,bdwth,difference)
    m = m+1 
    if(m>emmax) break()
  }   
  ...
  return(list(b=gamma.new,h=h.new,beta=beta.hat,iteration=iteration,survival=s,cured=w,pred=cbind(index,p_X),h.conv=h.CV$convergence,conv=incidence_est$convergence)) 
